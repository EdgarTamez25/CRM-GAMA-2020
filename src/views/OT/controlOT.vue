		...
		<v-row justify="center">
			<v-col cols="12" >
				...
				<v-card-actions class="pa-0" >
					...
					<v-spacer></v-spacer>
					<v-btn color="error" filled @click="$emit('modal',false)" text><v-icon>clear</v-icon></v-btn>
				</v-card-actions>
          ...
          <v-col cols="12" sm="6">
            <v-text-field
              v-model="referencia" label="Referencia" hide-details dense filled clearable
            ></v-text-field>
          </v-col>
  ...
  import loading     from '@/components/loading.vue';
	...
	export default {
      ...
      consultaDetalle(id){
        ...
        payload.id_cliente   = this.cliente.id,
        payload.oc			     = this.orden_compra,
        payload.referencia   = this.referencia,
