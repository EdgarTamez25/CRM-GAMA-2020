								...
								<template>
									<thead>
										<tr>
											...
											<th class="text-right">{{ precio }} {{ Moneda }}</th>
										</tr>
										<tr>
											<th class="text-left">C.ADMIN</th>
											<th class="text-right"> {{ Costo_Administrativo }} {{ Moneda }}</th>
										</tr>
										<tr>
										...
										<tr>
											...
											<th class="text-right">{{ TOTAL }} {{ Moneda }}</th>
										</tr>
									</thead>
								</template>
							</v-simple-table>
            ...
            >
              <template v-slot:selection="data">
                ...
                >
					...
					<v-col cols="12" v-if="tipo_producto === 2 && MP_Seleccionada.length">  <!-- MATERIAS PRIMAS A SELECCIONAR -->
						<v-card class="mx-auto" flat >
							<v-data-table hide-default-footer fixed-header dense :headers="headers" :items="DETALLE" v-if ="MP_Detalle">
								<template v-slot:item.cantidad="props">
									<v-edit-dialog 
										:return-value.sync="props.item.cantidad"  persistent 
										save cancel open close 
									>
										<div>{{ props.item.cantidad }}</div>
										<template v-slot:input >
											<v-text-field v-model="props.item.cantidad"   label="Cantidad" autofocus color="rosa" class="mt-2"></v-text-field>
										</template>
									</v-edit-dialog>
								</template>
							</v-data-table>						
	...
	export default {
	  ...
	  data () {
			return {
				...
				headers:[ { text: 'Cant'		, align: 'start', value: 'cantidad'},
									{ text: 'Producto', align: 'start', value: 'nombre'},
		...
		computed:{
			...
			DETALLE:function(){
				console.log('ENTRE DETALLE');
				var detalle = [];
				for(let i=0; i<this.MP_Seleccionada.length;i++){
					for(let j=0; j<this.materia_prima.length;j++){
						if(this.MP_Seleccionada[i] === this.materia_prima[j].nombre){
							detalle.push({ id:          this.materia_prima[j].id,
														 nombre:  		this.materia_prima[j].nombre,
														 descripcion:	this.materia_prima[j].descripcion,
														 cantidad:    this.busCantidad(this.materia_prima[j].nombre)
														})
						}
					}
				}
				console.log('DETALLE', detalle);
				this.MP_Detalle = detalle;
				return detalle;
			},
			...
			Costo_Administrativo: function(){  // CALCULO DEL COSTO ADMINISTRATIVO
				...
				if(this.predeterminada.codigo === this.Moneda){  // COMPARO SI EL PRECIO ESTA EN FUNCION DE LA MONEDA PREDETERMINADA
					...
					this.costo_admin = (this.precio *  (this.pjeAdmin/100)) // SI LO ESTA SOLAMENTE SUMO
					...
					if(this.Moneda === 'MX'){ 
						...
						this.costo_admin = (this.precio *  (this.pjeAdmin/100)) // SUMO EL PRECIO YA CONVERTIDO
					}else{
						...
						this.costo_admin = (precio *  (this.pjeAdmin/100)) // SUMO EL PRECIO YA CONVERTIDO
			...
			TOTAL: function(){ // CALCULO DEL TOTAL DEL RESUMEN.
				...
				if(this.predeterminada.codigo === this.Moneda){
					...
					this.total = parseFloat(this.precio) + (this.precio *  (this.pjeAdmin/100))
					...
					if(this.Moneda === 'MX'){ // SI EL PRECIO ESTA EN MONEDA MX ENTONCES SOLO SOLO LA CANTIDAD
						...
						this.total = parseFloat(this.precio) + (this.precio *  (this.pjeAdmin/100)) // SUMO EL PRECIO YA CONVERTIDO
						...
						this.total =  parseFloat(precio) + (precio *  (this.pjeAdmin/100)) // SUMO EL PRECIO YA CONVERTIDO
					}
				}
		...
		methods:{
			...
			busCantidad(nombre){
				var cant = 0;
				for(let x=0;x<this.DetalleOrg.length;x++){
					if(this.DetalleOrg[x].nombre === nombre){ cant = this.DetalleOrg[x].cantidad }
				}
				return cant;
			},
			...
			consultaDetalleProducto(){
				this.$http.get('detalle_productos/'+ this.edit2.id ).then((response)=>{
					...
					for( var d in response.body){ this.DetalleOrg.push(response.body[d])}
			...
			PrepararPeticion(){
				...
				const payload = { 
													detalleActual: this.DetalleActual, //GUARDA EL DETALLE QUE CONSULTE
													...
													detalle     : this.MP_Detalle,   // DETALLE UNA VEZ MODIFICADO
			...
			CrearPrecio(payload){ // ESTA FUNCION MANDAR A INSERTAR EL PRECIO Y LOS DETALLES
				...
				console.log('insertar precio', payload)
