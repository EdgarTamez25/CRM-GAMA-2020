		...
		<v-row justify="center">
			<v-col cols="12">
				...
				<v-card-actions class="pa-0" >
					<h3> <strong> {{ param === 1? 'Nuevo Tipo de precio':'Editar Tipo de precio' }} </strong></h3> 
					<v-spacer></v-spacer>
	...
	export default {
	  ...
	  data () {
		...
		computed:{
			...
			...mapGetters('TiposPrecios' ,['getTiposPrecios']), 
		...
		methods:{
			...
			...mapActions('TiposPrecios'  ,['consultaTiposPrecios']),
			...
			PrepararPeticion(){
				...
				this.param === 1 ? this.CrearTiposPrecios(payload): this.ActualizarTiposPrecios(payload);
			},

			CrearTiposPrecios(payload){
				// ACTIVO DIALOGO -> GUARDANDO INFO
				this.dialog = true ;
				setTimeout(() => (this.dialog = false), 2000)
				
				// MANDO A INSERTAR ZONA
				this.$http.post('TiposPrecios', payload).then((response)=>{
					this.TerminarProceso(response.body);					
				})
			},

			ActualizarTiposPrecios(payload){
				// ACTIVO DIALOGO -> GUARDANDO INFO
				this.dialog = true ; this.textDialog ="Actualizando Información"
				setTimeout(() => (this.dialog = false), 2000)

				this.$http.put('TiposPrecios/'+ this.edit.id, payload).then((response)=>{
					this.TerminarProceso(response.body);					
				})
			},
			...
			TerminarProceso(mensaje){
				...
				this.consultaTiposPrecios() //ACTUALIZAR CONSULTA
			},
